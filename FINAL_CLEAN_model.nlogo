...
to setup-turtles
  create-turtles 20 [
    ...
    ;Setting group sizes:
    set gs round( random-normal gs_mean gs_sd )                     ; Pulling random group size from a normal distribution with global mean and sd of group size.
    while [ gs < 2 ] [                                              ; Safety check - we don't want gs' of 1 or smaller:
      set gs round( random-normal gs_mean gs_sd )                   ; Keep drawing new gs until we get all gs in the agentset >= 2.
    ]

    ; Setting male-to-female ratios:
    set male-percentage ( random-normal mean-ingroup-male-percentage 5 )
    set n-males round( male-percentage * gs / 100 )                ; Setting the number of males in the group based on the global mf_ratio.
    set male-percentage ( n-males / gs * 100 )                     ; Updating the male-percentage, since we rounded off the n-males.


    set opponent nobody                                            ; Groups start out with having no opponents.
